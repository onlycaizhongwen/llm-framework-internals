/**
 * Plain node-type-definition lookup helpers.
 *
 * Returns the full TypeScript type definition for a specific node from the
 * generated workflow-sdk types. No LangChain dependency — the LangChain
 * `tool()` wrapper lives in `@n8n/ai-workflow-builder` and re-uses
 * `getNodeTypes()` from this module.
 */
...
/**
 * Look up TypeScript type definitions for one or more nodes.
 *
 * Accepts simple node id strings or objects carrying discriminators
 * (resource/operation/mode). Returns a single formatted string containing
 * the definitions for all successfully-resolved nodes, with any errors
 * appended in an "# Errors" section.
 */
