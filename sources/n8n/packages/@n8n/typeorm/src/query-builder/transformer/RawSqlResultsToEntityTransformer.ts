...
export class RawSqlResultsToEntityTransformer {
		...
		if (alias.metadata.tableType === 'view') {
			keys.push(
				...alias.metadata.columns.map((column) =>
					DriverUtils.buildAlias(this.driver, undefined, alias.name, column.databaseName),
				),
			);
		} else {
			keys.push(
	...
	protected transformRawResultsGroup(rawResults: any[], alias: Alias): ObjectLiteral | undefined {
		...
		const hasOnlyVirtualPrimaryColumns =
			metadata.primaryColumns.filter((column) => column.isVirtual === false).length === 0; // todo: create metadata.hasOnlyVirtualPrimaryColumns
		...
		this.rawRelationCountResults
			...
			.forEach((rawRelationCountResult) => {
				...
				const referenceColumnValue =
					rawSqlResults[0][
						DriverUtils.buildAlias(this.driver, undefined, alias.name, referenceColumnName)
			...
			rawSqlResults.forEach((rawSqlResult) => {
				if (relation.isManyToOne || relation.isOneToOneOwner) {
					valueMap[column.databaseName] = this.driver.prepareHydratedValue(
						rawSqlResult[
							DriverUtils.buildAlias(this.driver, undefined, parentAlias, column.databaseName)
						],
						column,
					);
				} else {
