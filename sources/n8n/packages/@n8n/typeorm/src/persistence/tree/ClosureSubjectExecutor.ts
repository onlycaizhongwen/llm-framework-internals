...
export class ClosureSubjectExecutor {
	...
	async update(subject: Subject): Promise<void> {
		...
		await this.queryRunner.manager
			...
			.where(
				(qb) => `(${descendantColumnNames.join(', ')}) IN (${createSubQuery(qb, 'descendant')})`,
			)
			.andWhere(
				(qb) => `(${ancestorColumnNames.join(', ')}) NOT IN (${createSubQuery(qb, 'ancestor')})`,
			)
		...
		if (parent) {
			...
			const entityWhereCondition = subject.metadata.closureJunctionTable.ancestorColumns.map(
				(column) => {
					const columnName = escape(column.databaseName);
			...
			const parentWhereCondition = subject.metadata.closureJunctionTable.descendantColumns.map(
				(column) => {
					const columnName = escape(column.databaseName);
					const parentId = column.referencedColumn!.getEntityValue(parent);
