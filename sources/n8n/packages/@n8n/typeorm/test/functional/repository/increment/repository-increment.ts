		...
		it('should increment value', () =>
			Promise.all(
				connections.map(async (connection) => {
					...
					await connection.getRepository(Post).increment({ id: 1 }, 'counter', 1);

					// increment counter of post 2
					await connection.manager.increment(Post, { id: 2 }, 'counter', 3);
		...
		it('should accept string as input and increment value', () =>
			Promise.all(
				connections.map(async (connection) => {
					...
					// increment counter of post 1
					await connection.getRepository(Post).increment({ id: 1 }, 'counter', '22');

					// increment counter of post 2
					await connection.manager.increment(Post, { id: 2 }, 'counter', '33');
		...
		it('should return UpdateResult', () =>
			Promise.all(
				connections.map(async (connection) => {
					...
					// increment counter of post 1
					const result = await connection.getRepository(Post).increment({ id: 1 }, 'counter', 22);
		...
		it('should throw an error if column property path was not found', () =>
			Promise.all(
				connections.map(async (connection) => {
					...
					// increment counter of post 1
					await connection.getRepository(Post).increment({ id: 1 }, 'unknownProperty', 1).should.be
						.rejected;
				}),
			));
		...
		it('should throw an error if input value is not number', () =>
			Promise.all(
				connections.map(async (connection) => {
					...
					// increment counter of post 1
					await connection.getRepository(Post).increment({ id: 1 }, 'counter', '12abc').should.be
						.rejected;
				}),
			));
	});
