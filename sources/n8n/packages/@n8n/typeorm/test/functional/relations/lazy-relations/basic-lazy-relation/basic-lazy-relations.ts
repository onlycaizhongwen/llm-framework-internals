...
describe('basic-lazy-relations', () => {
	...
	// Resolve fixtures relative to this file's source location. The JSON
	// schemas live next to the source test, not the compiled output, so strip
	// the build/compiled segment to point back at the source directory.
	const resourceDir = __dirname.replace(/[\\/]build[\\/]compiled[\\/]/, '/') + '/';
	...
	it('should successfully load relations within a transaction', () =>
		Promise.all(
			connections
				.filter((connection) =>
					new Set(['sqlite', 'sqlite-pooled', 'postgres']).has(connection.options.type),
				)
