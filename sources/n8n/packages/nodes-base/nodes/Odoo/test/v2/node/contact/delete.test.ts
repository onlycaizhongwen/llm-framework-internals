...
describe('OdooV2 — contact:delete', () => {
	let node: OdooV2;
	...
	it('deletes a contact and returns id + deleted flag', async () => {
		setupParams();
		(transport.odooApiRequest as Mock).mockResolvedValue(true);

		const result = await node.execute.call(exec);

		// unlink() — ids scoped to recordset by /json/2, no extra kwargs needed
		expect(transport.odooApiRequest).toHaveBeenCalledWith('res.partner', 'unlink', {
			ids: [CONTACT_ID],
		});
		expect(result[0][0].json).toEqual({ id: CONTACT_ID, deleted: true });
	});
