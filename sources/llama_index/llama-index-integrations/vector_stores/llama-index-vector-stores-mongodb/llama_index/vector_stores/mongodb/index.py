...
async def acreate_vector_search_index(
    ...
    **kwargs: Any,
    ...
    if wait_until_complete:
        ...
        async def _predictate() -> bool:
            return await _ais_index_ready(collection, index_name)

        await _await_for_predicate(
            predicate=_predictate,
            err=f"{index_name=} did not complete in {wait_until_complete}!",
            timeout=wait_until_complete,
        )
    logger.info(result)
...
async def adrop_vector_search_index(
    ...
    *,
    ...
    Args:
        collection (AsyncCollection): MongoDB Collection with index to be dropped
        index_name (str): Name of the MongoDB index
        wait_until_complete (Optional[float]): If provided, number of seconds to wait
            until search index is ready.
...
async def _ais_index_ready(collection: AsyncCollection, index_name: str) -> bool:
    """
    Check for the index name in the list of available search indexes.
...
async def _await_for_predicate(
    predicate: Awaitable, err: str, timeout: float = 120, interval: float = 0.5
) -> None:
    """
    Generic to block until the predicate returns true.
