...
def test_vectorstore(
    ...
    with lock:
        ...
        # 1a. Wait for the documents to be indexed
        try:
            vector_store.create_vector_search_index(
                path="embedding", dimensions=1536, similarity="cosine", filters=["text"]
            )
            vector_store.create_fulltext_search_index(field="text", field_type="string")
...
@pytest.mark.asyncio
async def test_vectorstore_async(
    nodes: List[TextNode],
    vector_store: MongoDBAtlasVectorSearch,
    embed_model: OpenAIEmbedding,
) -> None:
    """Test async add, query, delete API of MongoDBAtlasVectorSearch."""
    with lock:
        # 0. Clean up the collection
        await vector_store.async_collection.delete_many({})
        await asyncio.sleep(2)

        # 1a. Wait for the documents to be indexed
        try:
            await vector_store.acreate_vector_search_index(
                path="embedding", dimensions=1536, similarity="cosine", filters=["text"]
            )
            await vector_store.acreate_fulltext_search_index(
                field="text", field_type="string"
            )
        except Exception as e:
            pass
        ...
        query_str = "What are LLMs useful for?"
        n_similar = 2
        query_embedding = await embed_model.aget_text_embedding(query_str)
        query = VectorStoreQuery(
            query_embedding=query_embedding,
            similarity_top_k=n_similar,
        )
        ...
        retries = 5
        while retries and not result_found:
            query_responses = await vector_store.aquery(query=query)
            if len(query_responses.nodes) == n_similar:
                result_found = True
            else:
                await asyncio.sleep(2)
        ...
        responses_with_filter = await vector_store.aquery(query=query)
        assert len(responses_with_filter.ids) == n_similar
        assert all(
            filters.filters[0].value not in node.text
            for node in responses_with_filter.nodes
        ...
        responses_with_filter_compound = await vector_store.aquery(query=query)
        assert len(responses_with_filter_compound.ids) == n_similar
        assert all(
            ftext not in node.text
            for node in responses_with_filter_compound.nodes
        ...
        retries = 5
        while retries and not result_found:
            fulltext_result = await vector_store.aquery(query=query)
            if fulltext_result.ids:  # if len(fulltext_result.nodes) == n_similar:
                result_found = True
            else:
                await asyncio.sleep(2)
        ...
        hybrid_result = await vector_store.aquery(query=query)
        assert len(hybrid_result.ids) == n_similar
        assert not all("LlamaIndex" in node.text for node in hybrid_result.nodes[:3])
        assert not all("LLM" in node.text for node in hybrid_result.nodes[:3])
        ...
        # so deleting the ref_doc_id from any node
        # should delete ALL the nodes.
        n_docs = await vector_store.async_collection.count_documents({})
        assert n_docs == len(ids)
        remove_id = query_responses.nodes[0].ref_doc_id
        await asyncio.sleep(2)
        retries = 5
        while retries:
            await vector_store.adelete(remove_id)
            n_remaining = await vector_store.async_collection.count_documents({})
            if n_remaining == n_docs:
                await asyncio.sleep(2)
                retries -= 1
            else:
