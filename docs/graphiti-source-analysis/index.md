...
### 10.1 更真实的业务例子：客户偏好随时间变化

假设一个旅游助理 Agent 记录用户偏好。第一天用户说：
...
Graphiti 写入第一条 episode 时，可能形成：

| 图谱对象 | 示例 |
| --- | --- |
| Episode | “我喜欢行程排满一点，酒店便宜就行。” |
| Entity | 用户、行程、酒店 |
| Fact edge | 用户 偏好 行程排满 |
| Fact edge | 用户 偏好 便宜酒店 |
| 时间 | `valid_at = 第一天对话时间` |

写入第二条 episode 后，Graphiti 不应该简单覆盖旧事实，而是形成新的事实，并让冲突旧事实失效：

| 当前事实 | 历史事实 |
| --- | --- |
| 用户这次带孩子，偏好不太赶的行程 | 用户曾经偏好行程排满 |
| 用户这次偏好安静酒店 | 用户曾经更关注酒店便宜 |

这就是 `invalid_at` 的价值：当 Agent 问“这次应该怎么安排？”时，应使用当前事实；当用户问“我之前是不是一直喜欢特种兵行程？”时，又能追溯历史事实。
